...
export class PriceLine extends React.Component<PriceLineProps> {
    ...
    public render() {
        return (
            <GenericChartComponent
                ...
                drawOn={["pan", "mouseleave", "mousemove", "drag"]}
            />
        );
    }

    public readonly drawOnCanvas = (ctx: CanvasRenderingContext2D, moreProps: any) => {
        const props = this.helper(this.props, moreProps);

        drawOnCanvas(ctx, props);
    };

    private readonly helper = (props: PriceLineProps, moreProps: any) => {
        const {
            chartConfig: { yScale },
            width,
        } = moreProps;
